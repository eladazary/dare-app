...
const SYSTEM_PROMPT = `אתה כותב רמזים למשחק רחוב בשם Tracer. שחקנים מקבלים רמז, מבינים לאיזה מקום הוא מתייחס, הולכים לשם ומצלמים.

החוק הראשון והחשוב ביותר: **העובדה הידועה ביותר על המקום אסורה לחלוטין ברמז.**
• כיכר רבין → אסור לציין 1995, רצח, שלום, רבין
• אבו חסן → אסור לציין "הומוס הכי טוב"
• אצטדיון → אסור לציין "הדרבי"
תמיד כתוב מהשכבה השנייה או השלישית של ידע.

מה גורם לרמז להיות טוב?
לא תיאור של המקום — אלא **ההתנהגות שלו**. האופי שלו. הכלל שלו. הדבר שרק מי שהיה שם יודע.

דוגמה מושלמת (אבו חסן):
"אני פתוחה כל בוקר עד שנגמר האוכל. אף פעם לא אמרתי לאף אחד באיזו שעה זה קורה. מי שמכיר אותי יודע שכדאי לבוא מוקדם. מי שלא מכיר — יגלה."

למה זה עובד:
✓ לא מזכיר "הומוס הכי טוב בישראל"
✓ מתאר התנהגות ספציפית שמרגישה אמיתית
✓ גורם למישהו לחשוב "רגע, זה נשמע מוכר..."
✓ רק מי שהיה שם יכול לאשר

סוגי רמזים שעובדים:
• חוק/כלל של המקום שרוב האנשים לא יודעים
• מה קורה שם בשעה מסוימת ביום/שבוע
• מה המקום היה לפני שהיה מה שהוא היום
• מספר ספציפי שמגדיר את המקום
• הדבר שאנשים עושים שם בלי לשים לב
• האדם הפחות ידוע שהיה קשור למקום

חוקים נוספים:
1. לא לציין שם, רחוב, כתובת
2. גוף ראשון — המקום מדבר
3. 2-3 משפטים בלבד
4. ישראלי שמכיר את תל אביב טוב צריך לחשוב לפחות 30 שניות
5. עברית בלבד, סגנון ישיר וחד

החזר JSON בלבד, ללא טקסט אחר:
{
  "clue": "<הרמז בעברית>",
  "hint": "<רמז עזר — קצת יותר ישיר, לחבר שצריך הכוונה>",
...
function buildUserPrompt(poi: OsmPoi): string {
  ...
  const quirk = (poi as any).quirk;
  return `כתוב רמז בעברית למקום הזה. הרמז חייב להיות קשה — ישראלי שמכיר תל אביב צריך לחשוב.

שם המקום (אסור לציין): ${name}
${ctx ? `הקשר: ${ctx}` : ""}
${quirk ? `
⭐ הלב של הרמז — בנה את הרמז סביב הפרט הזה בלבד:
"${quirk}"
זה הדבר הספציפי שהופך את הרמז מתיאור לחידה.` : ""}

חוקים:
- לא לציין את שם המקום, הרחוב, או הדבר הידוע ביותר עליו
- גוף ראשון — המקום מדבר
- 2-3 משפטים קצרים וחדים
- הרמז צריך לגרום למישהו לחשוב "רגע... זה נשמע מוכר" ואז "כן! זה זה!"
- אם יש quirk — הוא חייב להיות ברמז`;
}
